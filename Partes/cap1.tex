...
%Furthermore, there are several repeated entries on the PDB, due to different studies on the same protein, further spreading the difference between the number of sequenced and structured proteins. 
...
There are two main approaches to solving the \ac{PSPP}: Knowledge-based methods and
...
an energy function. The main advantage of \text{Ab Initio} over the other methods
...
%MHs have been employed to solve several real world hard problems, such as face recognition~\cite{plichoski2018adjustable}, data mining~\cite{parpinelli2002data}, truss optimization~\cite{cazacu2014steel} and cancer detection~\cite{cazacu2014steel}.
...
%A hybridization method found throughout the literature is the Monte Carlo (MC) procedure~\cite{a}. It consists of a probabilistic random sampling of the search space.
%For the particular case of \textit{ab initio} protein prediction, a MC
%procedure can be employed to replace small (for instance, 3 or 9) continuous segments of the
%protein with know pieces of other proteins which may be similar to the target protein. This
%is know as fragment insertion~\cite{a}, and when coupled with a MC procedure it is a MC based
%fragment insertion.
...
% Several MC based fragment insertion procedures will be utilized.
...
% Develop a DE into a competitive method
% Utilize parameter adaptation
% Utilize fragments
% Benchmark it on several proteins
