...
For this work, a simple Evolutionary Algorithm is employed due to its capability
...
In~\cite{kim2009sampling} the author states that the bottleneck of solving the
...
actually controler the \ac{MC} temperature parameter. The parameter control is
...
post processing phase starts. The first postprocessing step is to cluster the
conformations. After the clustering process finishes, the cluster centroids are
selected. The number of clusters are a parameter of the algorithms.
The conformations closest to the centroids are found and feed forward
to the next steps, while the centroids themselves are discarted. A Hooke-Jeeves
...
for a human specialist to inspect multiple conformations that are appart from
