...
the \ac{DNA}. The \ac{DNA} encodes an amino acid sequence that when assembled
correctly will originate the desired protein.
Upon creation and shortly after the protein
...
its amino acid sequence, it is possible that the protein can fold incorrectly.
...
The building blocks of proteins are amino acids. There are 20 canonical amino
acids with varying degrees of complexity. These 20 amino acids are found
encoded in the genetic material of several organisms. Many more amino acids
exists, some of which only can happen synthetically, and others that are
intermediates of metabolical pathways and only exist
briefly~\cite{wagner1983new}. Of the 20 canonical amino acids, the human body
can only synthesize 10, and the other 10 must be externally supplied (in the
food). The lack of any of these amino acids will cause diminishing body
functions, as shown in~\cite{crews1969weight}.
...
function inside a protein and is of valuable use in this work.

% Peptide bonds
Two amino acids can bound together in order to form a peptide via a peptide
bound. This bound happens between the amino group of an amino acid
...
nitrogen of the amino group. This process can happen several times, and each
time it happens a new amino acid is aggregated to the chain and a water
molecule is a byproduct of the process~\cite{garrett1999biochemistry}. A small
sequence of peptide bound together is called a peptide chain. Bigger sequences
are considered proteins.
...
When a peptide bond is formed and it starts to grow, then it will assume a three
...
The sequence of bonds $N - C_\alpha$, $C_\alpha - C$ and $C - N$ extending over the
full body of the protein is called the backbone.
This chain extends over all the protein and is responsible for the
...
% levels
The analysis of a protein happens with increasing levels of details. There are
four abstract levels of detail commonly used in proteomics to identify, study
and describe a given protein and its properties~\cite{tsai2003introduction}.
These abstractions are the primary, secondary, tertiary and quaternary
structures.
...
syntheses is complete it will represented as the sequence of amino acids in
the protein. The primary structure is unique for each protein and can be used
to uniquely identify it. For instance, the
...
amino acids are bonded to the amino acid three positions ahead. This implies in
...
$\pi$-helix, in which the hydrogen bonds happens 5 amino acids apart. There, each
...
chains runs in opposite directions, as represented in
...
holds two helix or $\beta$-sheets together. If a loop is short, makes a
quick $180^\circ$ and has internal hydrogen bounds then it is called a turn.
If the amino acid sequence is located at
the start or the end of a protein, then it is called a coil. Turns and loops together
...
The tertiary structure of a protein is directly dependent on the primary
...
While the secondary structure references amino acids only on specific locations
...
given protein. Knowing in advance the secondary
...
in works such as~\cite{dunbrack1993backbone},
\cite{ligabue2018everyone} and~\cite{borguesan2015apl}.
...
outputting noisy data. They also are expensive and require a considerable human
...
division between the models are the on and off lattice models.
...
publication it was not known if that particular model was a
...
Despite the on lattice model's simplicity, it still is an intractable problem on
...
is possible using off lattice models, that is, a model that is not constrained
by a lattice.

The AB Off lattice model can be considered as the simplest model, in
which the amino acids are represented as spheres that are either hydrophobic or
...
To fully represent the protein, there are two main models: The Backbone and side
...
aspects that the model abstracted away. For this reason the all atom coordinates
model can be employed. In this model all atoms are represented and can be
degree of freedom. Currently, this model is the most accurate one, at the
...
\textit{ab initio} methods can be seen as an optimization problem where the
...
molecules). It was originally designed for molecular dynamics simulations.
...
focus primarily on molecular dynamics simulations, but can provide energy scoring of
protein conformations as well. A more detailed discussion of the energy functions is
available at~\cite{dorn2014three}. In~\cite{narloch2016diversification} the authors
explored the differences among \ac{AMBER}, \ac{CHARMM} and Rosetta. A further
...
The Rosetta energy functions take into account both the physicochemical properties of the
...
\ac{REU}. More information about the Rosetta energy function is available
...
literature are bio inspired algorithms. The well established algorithms
...
template proteins is used to represent the conformation space. Then, the
...
final prediction is then ready. In the event of the target protein having the
...
% downsides
Both \ac{TM} and HM require that a suitable known protein to exist to be used as
template. Either a protein with evolutionary similarity for \ac{TM} or one with 
homologous sequence for HM. If such proteins do exist,
then these methods will have a very
...
generated as in the \ac{DE} algorithm shown in Algorithm~\ref{algo:sade}. On line
...
expected to be the point in the conformational space where it has the lowest
...
In~\cite{oliveira2017sade} a method named SADE-SPL, consisting of a data-mining
...
operator. The modeling consists of a custom variant of Rosetta's \texttt{score3} function
