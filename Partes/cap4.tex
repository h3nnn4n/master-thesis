...
RPPF-REMC and RPPF-MC. One of the methods proposed
...
is compared against the work from~\citeonline{silva2019self} and the Rosetta Ab Initio
Protocol.
...
However, for protein 1rop, RPPF-MC outperformed the other methods by a
...
sade-mc-ffi had the second best result, behind RPPF-MC, while RPPF-REMC
...
result from RPPF-REMC was better than the medians of all the other methods.
...
more visible. For 1rop, RPPF-REMC appears to have had a worst performance
...
methods had $2$ and $1$ rejection of $H_0$, for RPPF-MC and RPPF-REMC,
...
there is no clear trend. The proposed method RPPF-REMC had no rejection
of $H_0$, while RPPF-MC had two.
...
1rop, sade-remc was able to outperform rppf-remc. Going back to the boxplots,
this was expected. Interestingly, RPPF-MC outperformed sade-remc, which means
that RPPF-MC also outperformed RPPF-REMC.
...
sade-remc. On 1acw, sade-remc outperformed RPPF-REMC and was statistically
equivalent to RPPF-MC. For 1enh only RPPF-MC outperformed sade-remc. For 1rop
sade-remc outperformed RPPF-REMC. The only situation so far where all the
...
proposed methods outperformed rosetta. For 1ail only RPPF-REMC
outperformed rosetta. On 1rop, RPPF-MC outperformed rosetta, while
RPPF-REMC was outperformed. On 1crn, only RPPF-MC was outperformed
...
RPPF-REMC. On 1ail, only RPPF-MC outperformed rosetta. Overall,
...
characteristics. In blue, it is possible to see that rppf-remc has a very fast
...
can disrupt the mean energy of the population. For rppf-mc the convergence
starts slower than for rppf-remc, however, the best energy keeps improving
...
When comparing rppf-mc and rppf-remc, by looking at the best energy of the two
methods, there is a lead for rppf-remc during the first 50 thousand function
evaluations. However, eventually rppf-mc achieves a best result and keeps
improving for the bigger part of the process. For all 10 proteins this can be
observed. Furthermore, when considering the average, rppf-remc had the best
results in all cases.

Two of the proteins had a increase of the average populational energy over time.
This happened only for protein 2mr9 with rppf-mc. This is the reason that this
...
average energy for rppf-mc, after approximately 70 thousand function iterations
...
and~\ref{fig:1utg-rmsd-convergence} rppf-remc has a lower diversity. For 9 of
...
After that, it stays relatively stable. For rppf-mc, in some proteins, such as
...
rppf-remc is explained by its Replica Exchange mechanism, where one trajectory
...
With this analysis, is possible to detect that rppf-mc over has better
convergence for the best individual than rppf-remc. However, when considering
the average energy of the population, rppf-remc has the best results.
Nevertheless, rppf-mc would be an interesting candidate for expanding the
budget of function evaluations available, since it keeps improving for a
longer period of time than rppf-remc and contains a bigger overall diversity in
the population.

\section{Monte Carlo vs Replica Exchange Monte Carlo}
% Compare RPPF-MC and RPPF-REMC
...
most of the proteins. For 1rop, was noticed in previous analysis, rppf-remc was
...
several other proteins rppf-mc appears to have consistently reached lower
energies. In fact, by applying the Mann-Whitney test, rppf-mc outperformed
rppf-remc for proteins 1acw, 1ail, 1enh, 1rop and 2mr9.
...
Interestingly, while rppf-mc outperformed rppf-remc for 1 protein considering
...
from Section~\ref{sec:convergence-analysis}, where rppf-mc had a higher
diversity, a desirable property to match with clustering, it is safe to consider
rppf-mc as the better method of the two.
...
\begin{table}
  ...
  \begin{tabular}{r|r|c||r|c||r|c}
            & \multicolumn{2}{c}{classic-abinitio} & \multicolumn{2}{||c}{RPPF-MC} & \multicolumn{2}{||c}{RPPF-REMC} \\ \hline
...
mean above $1$ was on 1rop for RPPF-REMC, which had the worst performance
...
\begin{table}
  ...
  \begin{tabular}{r|c|c||c|c}
            & \multicolumn{2}{c}{RPPF-MC} & \multicolumn{2}{||c}{RPPF-REMC} \\ \hline
  ...
  \end{tabular}
  \caption{Processing times, in seconds, for RPPF-MC and RPPF-REMC}
...
difference noticed was the standard deviation for 1rop. For RPPF-MC,
which had a relatively good prediction performance on this protein had
a standard deviation around $124$. The other method, RPPF-REMC, had a
...
\begin{table}
  ...
  \end{tabular}
  \caption{Function evaluations spent on Hooke-Jeeves, for RPPF-MC and RPPF-REMC}
...
the best RMSD was achieved by RPPF-MC. On 2mr9 the proposed methods
...
for the two proposed methods, RPPF-REMC and RPPF-MC with their
...
\begin{table}
  ...
  \begin{tabular}{r|c|c|c||c|c|c}
            & \multicolumn{3}{c}{RPPF-MC} & \multicolumn{3}{||c}{RPPF-REMC} \\ \hline
  ...
  \end{tabular}
  \caption{GDT-TS for RPPF-MC and RPPF-REMC}
...
\begin{table}
  ...
  \end{tabular}
  \caption{TM Score for RPPF-MC and RPPF-REMC}
...
than two units apart with RPPF-MC. However, with the GDT-TS for the same
...
extensive. For this reason, RPPF-MC was selected to be inspected, since
...
predictions by RPPF-MC are presented. The proteins are presented in
